...
RPsolvers::RPsolvers(double c, eulerTests Test, int nU, int nF)
	: CFL(c), N(Test.N), count(0), dt(0), dx(Test.L/Test.N), X(nU, 1), U(nU, 3), F(nF, 3){
}
...
void HLLC::initial_conditions(JWL MG, eulerTests Test){
	vector eulerL;
	vector eulerR;
...
void HLLC::solver(JWL MG, eulerTests Test){
	...
	//Davies speed is used for simplicity
...
void HLLC::output(JWL MG){
	...
	std::cout << "done: HLLC - JWL" << std::endl;
}
...
MUSCL::MUSCL(double c, eulerTests Test)
	:RPsolvers(c, Test, Test.N+4, Test.N+2){
}
...
void MUSCL::solver(JWL MG, eulerTests Test){
	...
	do{
		...
		for (int i=1; i<N+2; i++){
			...
			vector ULbar = ULtmp1 + 0.5*(dt/dx)*(MG.f(ULtmp1) - MG.f(URtmp1)); //UL(i+1)
			vector URbar = URtmp + 0.5*(dt/dx)*(MG.f(ULtmp) - MG.f(URtmp));
...
void MUSCL::output(JWL MG){
	...
	std::cout << "done: MUSCL - JWL" << std::endl;
}
