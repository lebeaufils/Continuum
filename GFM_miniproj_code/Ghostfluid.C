...
void GhostFluidMethods::initial_conditions(EOS* eos1, EOS* eos2, EOS* eos3, gfmTests Test){
	...
	if (Test.yM1 != 0){
		...
		throw "Number of materials match number of EOS parameters.";
	}
...
void GhostFluidMethods::solver(gfmTests Test){
	...
	if (Test.number_of_materials == 2){
		initial_conditions(eos1, eos2, Test);
		do{
		...
		initial_conditions(eos1, eos2, eos3, Test);
		do{
			//compute ghost fluid boundaries
			for (int i=1; i<N+1; i++){
				int testsgn = (get_sgn(phi(i)) + get_sgn(phi(i+1)));
		...
		initial_conditions(eos1, eos2, eos3, eos4, Test);
		do{
			//compute ghost fluid boundaries
			bool flag1 = false; //Markers to identify which domain have been computed
			bool flag2 = false;
...
void GhostFluidMethods::initial_conditions_RP(EOS* eos1, EOS* eos2, EOS* eos3, EOS* eos4, gfmTests Test){
	//setting material EOS parameter
	eos1->y = Test.yL;
	eos4->y = Test.yR;
	if (Test.yM1 != 0 && Test.yM2 != 0){
	...
	for (int i=0; i<N; i++){
		...
		//std::cout << i << '\t' << get_sgn(phi(i+1)) << '\t' << var1->U.row(i+2) << '\t' << var2->U.row(i+2) << std::endl;
		//std::cout << phi(i+1) << std::endl;
	}
		...
		if (testsgn > -2 && testsgn < 1 && i<N){ //-1 or 0, the boundary is crossed
			if (get_sgn(phi(i)) < 0){//the interface is to the right of cell i
				...
				//the materials now obey eos3 in var1 and eos4 in var2
					...
					ghost_boundary_RP(var1, eos3, var2, eos4, i+1);
					flag3 = true;
				}
			}
			else {//the interface is to the left of cell i+1 (i+1 is negative while i is positive)
	...
	if (Test.number_of_materials == 2){
		...
		do{			
			...
			for (int i=1; i<N+2; i++){
				...
				//if (count==0)std::cout << i << '\t' << phi(i)  << '\t' << var1->U.row(i+1) << '\t' << '\t' << var2->U.row(i+1) << std::endl;
				//if (count==0)std::cout << i << '\t' << phi(i) << '\t' << var1->F.row(i) << '\t' << '\t' << var2->F.row(i) << std::endl;
			}
		...
		EOS* eosM1 = new IdealGas();
		EOS* eosM2 = new IdealGas();
		initial_conditions_RP(eosL, eosM1, eosM2, eosR, Test);
				...
				if (testsgn > -2 && testsgn < 1 && i<N){ //-1 or 0, the boundary is crossed
					if (get_sgn(phi(i)) < 0){//the interface is to the right of cell i
						...
						//the materials now obey eos3 in var1 and eos4 in var2
							...
							ghost_boundary_RP(var1, eosM2, var2, eosR, i+1);
							flag3 = true;
						}
					}
					else {//the interface is to the left of cell i+1 (i+1 is negative while i is positive)
						//to the left of interface, the ghostfluid is in var1
						if(flag1 == true && flag2 == false && flag3 == false){
							ghost_boundary_RP(var2, eosM1, var1, eosM2, i+1);
							flag2 = true;
		...
		std::cout << count << std::endl;
		output(eosL, eosM1, eosM2, eosR);

		delete eosM1; delete eosM2;		
	}
...
void GhostFluidMethods::exact_solver(gfmTests Test){
	...
	if (Test.number_of_materials == 2){
		...
		//std::cout << dshockL << '\t' << dshockR << std::endl;
	...
	//This only works for a stationary second shock such as fedkiw's test B and C
